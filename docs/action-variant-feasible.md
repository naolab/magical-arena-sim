...
- [x] **被ダメージ増加デバフ** ✅ 実装済み (Terror damage_amplify)
  - 実装: 敵に与ダメージ+20%のデバフ（3ターン）を付与（毒/呪い等にも適用）
  - 重ね掛け不可で再付与時は上書き
- [x] **コメント枯渇勝利トリガー** ✅ 実装済み (Terror victory_trigger)
  - 実装: 自分に永続効果を付与し、コメントが0になると即座に勝利（相打ち可）
  - バフ/デバフにカウントされず、クレンジングの対象外
...
- [x] **コメント上限縮小ヒール** ✅ 実装済み (Grief limit_heal)
  - 実装: コメント最大数を1ずつ減らしながら300回復（最大5回）
  - コメント上限は永続的に減少する
- [x] **デバフ依存ヒール** ✅ 実装済み (Grief debuff_heal)
  - 実装: 自分のデバフ1つにつき追加回復（例: 基礎200 + デバフ×50, 上限5個）
  - デバフ数を参照するだけで、効果は回復のみ
...
- [x] **コメント2色陣** ✅ 実装済み (Ecstasy dual_refresh)
  - 実装: コメントプールをリフレッシュし、2属性のみで再配置（低確率でスパチャも混入）
